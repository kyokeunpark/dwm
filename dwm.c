...
enum { NetSupported, NetWMName, NetWMState, NetWMCheck,
       ...
       NetWMWindowType, NetWMWindowTypeDock,
...
drawstatusbar(Monitor *m, int bh, char* stext) {
	...
	w = w + 2; /* 1px padding on both sides */
	ret = x = m->ww - w - stw;
	...
	if (!isCode) {
		...
		drw_text(drw, x - 2 * sp, 0, w, bh, lrpad / 2 - 2, text, 0);
	}
...
updatesystray(void)
{
	...
	for (w = 0, i = systray->icons; i; i = i->next) {
		...
		w += systrayspacing;
		i->x = w;
		XMoveResizeWindow(dpy, i->win, i->x + 3, 3, i->w - 3*2, i->h - 3*2);
		w += i->w;
		if (i->mon != m)
			i->mon = m;
	}
	...
	XSetBackground(dpy, drw->gc, scheme[SchemeNorm][ColBg].pixel);
